...
function BubbleScreen({selectedIndustries,onBack,onViewCareer}) {
  ...
  const positions=allCareers.map((_,i)=>{const a=(i/allCareers.length)*2*Math.PI;const r=28+(i%3)*12;return{x:50+r*Math.cos(a),y:50+r*Math.sin(a)};});
  function selectBubble(c){setActiveBubble(c);setVisited(p=>new Set([...p,c.title]));}
  return (
    <Screen>
      <button style={backStyle} onClick={onBack}>← Back</button>
      <div style={eyebrowStyle}>Career universe</div>
      <div style={{fontSize:12,color:T.textDim,marginBottom:12}}>Tap any bubble to explore</div>
      <div style={{position:"relative",width:"100%",height:380,background:T.bgDeep,borderRadius:18,border:`1px solid ${T.border}`,overflow:"hidden",marginBottom:12}}>
        {allCareers.map((c,i)=>{
          const pos=positions[i];const isActive=activeBubble?.title===c.title;const r=34+(c.title.length%3)*5;
          return(
            <div key={c.title} onClick={()=>selectBubble(c)} style={{position:"absolute",width:r*2,height:r*2,left:`calc(${pos.x}% - ${r}px)`,top:`calc(${pos.y}% - ${r}px)`,borderRadius:"50%",background:isActive?c.industryColor:c.industryBg,border:`2px solid ${c.industryColor}`,display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",opacity:activeBubble&&!isActive?0.35:visited.has(c.title)&&!isActive?0.65:1,transform:isActive?"scale(1.12)":"scale(1)",transition:"all 0.2s",textAlign:"center",padding:4}}>
              <span style={{fontSize:9,fontWeight:700,color:isActive?"#fff":c.industryColor,lineHeight:1.2}}>{c.title}</span>
            </div>
          );
        })}
      </div>
      {activeBubble&&(
        <div style={cardStyle}>
          <div style={{fontSize:15,fontWeight:800,color:T.text,marginBottom:4}}>{activeBubble.title}</div>
          <div style={{fontSize:12,color:T.textMid,lineHeight:1.5,marginBottom:10}}>{activeBubble.desc}</div>
          <div style={{display:"flex",gap:8,marginBottom:12}}>
            <span style={pillStyle(T.accent1)}>{activeBubble.salary}</span>
            <span style={pillStyle(T.accentPurple)}>{activeBubble.school}</span>
          </div>
          <button onClick={()=>onViewCareer(activeBubble,activeBubble.industryColor)} style={{...primaryStyle,fontSize:13,padding:"0.7rem"}}>See full career roadmap →</button>
        </div>
      )}
    </Screen>
  );
