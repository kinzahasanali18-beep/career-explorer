...
// Snapshot of the 1016 occupations in the O*NET-SOC taxonomy, taken from
// https://www.onetonline.org/find/all on 2026-08-10.
//
// Two jobs:
//   1. VALIDATION. A career's source_url is only trusted if it carries a SOC code
//      that exists here. The nightly generator hallucinated codes freely — 193 of
//      547 distinct O*NET URLs in the database point at codes that do not exist.
//      O*NET is the authority because its taxonomy is complete and checkable.
//   2. LABELS. Lets a citation name the occupation it cites ("Set and Exhibit
//      Designers") rather than showing a bare URL.
//
// CareerOneStop (a U.S. Department of Labor site) is the citation shown to
// students, because its occupation profile is far more readable than O*NET's —
// wages, outlook, video, local data. Its URLs are keyed by the same SOC code, so
// they are DERIVED here, never model-generated, and cannot be hallucinated.
// It omits the 76 "All Other" residual categories, so those fall back to O*NET.
//
// A blank title means the code is valid but its title could not be parsed from
// the source page; it still validates, it just cannot be labelled.

export const OCCUPATION_TITLES = {
...
const ONET_URL_RE = /onetonline\.org\/link\/summary\/(\d{2}-\d{4}\.\d{2})/;

// Pull the SOC code out of a stored source_url. Returns null for anything that
// is not an O*NET summary link — which is how every legacy bls.gov citation
// (2,778 rows, 61% of them broken) ends up showing no citation at all.
export function socFromSourceUrl(url) {
  const m = ONET_URL_RE.exec(String(url || ""));
  return m ? m[1] : null;
}

export function isValidSocCode(code) {
  return Object.prototype.hasOwnProperty.call(OCCUPATION_TITLES, String(code || ""));
}

export function occupationTitle(code) {
  return OCCUPATION_TITLES[String(code || "")] || "";
}

// CareerOneStop has no page for the "All Other" residual categories.
export function careerOneStopCovers(code) {
  const t = occupationTitle(code);
  return !!t && !/all other/i.test(t);
}

export function onetUrl(code) {
  return `https://www.onetonline.org/link/summary/${code}`;
}

// CareerOneStop keys occupations by the SOC code with punctuation stripped:
// 27-1027.00 -> 27102700
export function careerOneStopUrl(code) {
  const flat = String(code || "").replace(/[.-]/g, "");
  return `https://www.careeronestop.org/Toolkit/Careers/Occupations/occupation-profile.aspx?onetcode=${flat}&location=US`;
}

// Resolve a stored source_url into everything needed to render a citation, or
// null when the source cannot be trusted. Callers render nothing on null — a
// broken citation is worse than no citation.
export function resolveCitation(sourceUrl) {
  const code = socFromSourceUrl(sourceUrl);
  if (!code || !isValidSocCode(code)) return null;
  const title = occupationTitle(code);
  const useCos = careerOneStopCovers(code);
  return {
    code,
    title,
    href: useCos ? careerOneStopUrl(code) : onetUrl(code),
    provider: useCos ? "CareerOneStop" : "O*NET",
    onetHref: onetUrl(code),
  };
}
