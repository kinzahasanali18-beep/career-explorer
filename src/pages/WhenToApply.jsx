...
export const STATUS_CONFIG = {
  ...
  later:   { label: "Later",      emoji: "⚪", color: "#8B8FA8", bg: "#8B8FA818" },
};

const STATUS_ORDER = { open: 0, soon: 1, rolling: 2, later: 3 };
...
export default function WhenToApply({ starredItems, onToggleStar }) {
  ...
  const sorted = [...filtered].sort((a, b) => {
    if (sortBy === "industry") return a.n.localeCompare(b.n);
  ...
  return (
    <div className="sparq-screen" style={{ padding: "72px 1.25rem 90px", fontFamily: "'Inter',system-ui,sans-serif" }}>
      ...
      <div style={{
        ...
        fontSize: 11, color: "#D4A030", lineHeight: 1.6,
      }}>
        <strong>heads up</strong> — these dates shift every year and every company does its own thing. think of this as your heads-up, not gospel. always double-check the actual program or company page before you panic OR relax.
      </div>

      {/* Sort control */}
      <div style={{ display: "flex", gap: 6, marginBottom: 14, flexWrap: "wrap" }}>
        {[["urgent", "Urgent now"], ["industry", "By industry"]].map(([val, label]) => (
          <button
            key={val}
            onClick={() => setSortBy(val)}
            style={{
              padding: "5px 13px", borderRadius: 20, fontSize: 12, fontWeight: 600,
              border: `1px solid ${sortBy === val ? T.accent : T.border}`,
              background: sortBy === val ? `${T.accent}22` : "transparent",
              color: sortBy === val ? T.accent : T.textMid,
      ...
      {sorted.map(item => {
        ...
        const sc = STATUS_CONFIG[item.status] || STATUS_CONFIG.later;
