...
async function enrichCareer(name, industry, traits, salary) {
  const res = await fetch("https://api.anthropic.com/v1/messages", {
    ...
    body: JSON.stringify({
      ...
      max_tokens: 300,
      messages: [{ role: "user", content: `For the career "${name}" in ${industry} with traits ${traits}:
Return ONLY valid JSON, no markdown:
{"work_style":"Remote or Hybrid or In-person or Field-based","schedule_type":"Traditional 9-5 or Flexible or Shift-based or Project-based or Autonomous","work_environment":"Office or Lab or Studio or Outdoors or Travel-heavy or Varies","degree_required":"Yes or No or Sometimes","entry_level_friendly":"Yes or No","requirements":"1 sentence on education/certs needed"}` }]
    })
  });
  const data = await res.json();
  ...
  catch(e) { console.log("  Parse err:", raw.slice(0, 80)); return null; }
}
...
async function updateRecord(id, fields) {
  ...
  const data = await res.json();
  if (!res.ok) { console.log("  AT err:", JSON.stringify(data.error)); return false; }
  return true;
}

async function main() {
  console.log("🚀 Sparq Enrichment v2\n");
  ...
  for (let i = 0; i < careers.length; i++) {
    ...
    if (!enriched) { console.log("❌ Claude"); fail++; continue; }
